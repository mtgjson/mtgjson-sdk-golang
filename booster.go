...
func newBoosterSimulator(conn *Connection) *BoosterSimulator {
...
func (bs *BoosterSimulator) OpenPack(ctx context.Context, setCode, boosterType string) ([]CardSet, error) {
	...
	// Preserve pack order
	uuidToCard := make(map[string]CardSet, len(cards))
	for _, c := range cards {
		uuidToCard[c.UUID] = c
	}
	ordered := make([]CardSet, 0, len(cardUUIDs))
...
func (bs *BoosterSimulator) OpenBox(ctx context.Context, setCode, boosterType string, packs int) ([][]CardSet, error) {
	if packs <= 0 {
		packs = 36
	}
	box := make([][]CardSet, 0, packs)
	for i := 0; i < packs; i++ {
...
func pickFromSheet(sheet map[string]any, count int) []string {
	...
	for uuid, weightRaw := range cardsRaw {
		...
		weights = append(weights, toFloat64(weightRaw))
	}
