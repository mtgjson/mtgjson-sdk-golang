...
func newCardQuery(conn *Connection) *CardQuery {
	return &CardQuery{conn: conn}
}

// GetByUUID returns a single card by its MTGJSON UUID, or nil if not found.
func (q *CardQuery) GetByUUID(ctx context.Context, uuid string) (*CardSet, error) {
...
func (q *CardQuery) GetByUUIDs(ctx context.Context, uuids []string) ([]CardSet, error) {
	if len(uuids) == 0 {
		return []CardSet{}, nil
	}
	if err := q.conn.EnsureViews(ctx, "cards"); err != nil {
...
func (q *CardQuery) GetByName(ctx context.Context, name string, setCode ...string) ([]CardSet, error) {
	if err := q.conn.EnsureViews(ctx, "cards"); err != nil {
		return nil, err
	}
	b := NewSQLBuilder("cards").WhereEq("name", name)
...
func (q *CardQuery) Search(ctx context.Context, p SearchCardsParams) ([]CardSet, error) {
	if err := q.conn.EnsureViews(ctx, "cards"); err != nil {
		return nil, err
	}
	b := NewSQLBuilder("cards")
...
func (q *CardQuery) GetPrintings(ctx context.Context, name string) ([]CardSet, error) {
...
func (q *CardQuery) GetAtomic(ctx context.Context, name string) ([]CardAtomic, error) {
	if err := q.conn.EnsureViews(ctx, "cards"); err != nil {
		return nil, err
	}
	atomicCols := []string{
...
func (q *CardQuery) FindByScryfallID(ctx context.Context, scryfallID string) ([]CardSet, error) {
...
func (q *CardQuery) Random(ctx context.Context, count int) ([]CardSet, error) {
	if err := q.conn.EnsureViews(ctx, "cards"); err != nil {
		return nil, err
	}
...
func (q *CardQuery) Count(ctx context.Context, filters ...Filter) (int, error) {
	...
	b := NewSQLBuilder("cards").Select("COUNT(*)")
