...
func newDeckQuery(cache *CacheManager) *DeckQuery {
...
func (q *DeckQuery) List(ctx context.Context, params ListDecksParams) ([]DeckList, error) {
...
func (q *DeckQuery) Search(ctx context.Context, params SearchDecksParams) ([]DeckList, error) {
	if err := q.ensure(ctx); err != nil {
		return nil, err
	}
	results := q.data
...
func marshalDeckLists(data []map[string]any) ([]DeckList, error) {
	if len(data) == 0 {
		return nil, nil
	}
