...
func newSetQuery(conn *Connection) *SetQuery {
	return &SetQuery{conn: conn}
}

// Get returns a set by its code (case-insensitive), or nil if not found.
func (q *SetQuery) Get(ctx context.Context, code string) (*SetList, error) {
...
func (q *SetQuery) List(ctx context.Context, p ListSetsParams) ([]SetList, error) {
	if err := q.conn.EnsureViews(ctx, "sets"); err != nil {
		return nil, err
	}
	b := NewSQLBuilder("sets")
	if p.SetType != "" {
...
func (q *SetQuery) Search(ctx context.Context, p SearchSetsParams) ([]SetList, error) {
	if err := q.conn.EnsureViews(ctx, "sets"); err != nil {
		return nil, err
	}
	b := NewSQLBuilder("sets")
...
func (q *SetQuery) GetFinancialSummary(ctx context.Context, setCode string, opts ...FinancialSummaryOption) (*FinancialSummary, error) {
